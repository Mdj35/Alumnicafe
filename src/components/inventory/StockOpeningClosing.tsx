...
import { getInventoryItems, getOpeningStocks, saveOpeningStock, InventoryItem, OpeningStockEntry, OpeningStockItem } from '../../inventoryManager';
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
          {history.map(entry => (
            <div key={entry.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <div className="mt-3 text-[10px] text-gray-400 font-bold uppercase tracking-widest">
            ...
            <div className="p-6 border-b border-gray-100 flex justify-between items-start bg-white shrink-0">
              <div className="flex-1">
                <div className="flex items-center gap-4 mb-4">
                  <h2 className="text-2xl font-black text-gray-800 tracking-tight">Active Entry</h2>
              ...
              <div className="flex flex-col items-end gap-2 ml-4">
                ...
                <button onClick={() => setIsRecording(false)} className="text-xs font-bold text-gray-400 hover:text-gray-600">Cancel</button>
              </div>
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredItems.map(item => {
                    const qty = entryItems[item.id] ?? 0;
                    const val = qty * (item.unit_cost || 0);
                    const isDiff = qty !== (item.current_stock || 0);
